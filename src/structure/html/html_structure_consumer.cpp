...
std::unique_ptr<structure_consumer> html_structure_consumer::consume(events::exec_event const& e) {
  ...
  return std::make_unique<subconsumer>(e.source_pid, path);
}

html_structure_consumer::subconsumer::subconsumer(pid_t my_pid, std::filesystem::path filename)
    : my_pid(my_pid), filename(filename) {
...
std::unique_ptr<structure_consumer> html_structure_consumer::subconsumer::consume(events::exec_event const& e) {
  ...
  return std::make_unique<html_structure_consumer::subconsumer>(e.source_pid, subfilename);
}
